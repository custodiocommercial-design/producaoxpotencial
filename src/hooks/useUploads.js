...
import {
  MAPA_POTENCIAL, MAPA_LOJAS, MAPA_PRODUCAO,
  CAMPOS_NUMERICOS_LOJAS, CAMPOS_NUMERICOS_POTENCIAL, CAMPOS_NUMERICOS_PRODUCAO,
    ...
    try {
      ...
      const linhas = linhasBrutas
        .map((linha) => {
          ...
          if (!mapeada.filial) return null
          for (const campo of CAMPOS_NUMERICOS_LOJAS) {
            mapeada[campo] = paraNumero(mapeada[campo])
          }
          return mapeada
        })
      ...
      if (linhas.length === 0) {
        throw new Error('Nenhuma linha válida encontrada no arquivo (verifique se a coluna FILIAL está preenchida).')
      }
